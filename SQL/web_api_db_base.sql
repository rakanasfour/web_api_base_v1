...
CREATE TABLE packaging (
    ...
    packaging_type ENUM('SINGLE','PACK','BOX', 'CASE', 'PALLET') NOT NULL
);

#who is access the database #customer or wholesaler, dropshiper
...
item_a_item_id int UNSIGNED unique, 
foreign key (item_a_item_id) references items(item_id),

item_a_attribute_id int UNSIGNED unique, 
...
CREATE TABLE uom_hierachy (
    uom_hierachy_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    test varchar(250),
	test1 varchar(250)
);
...
FROM 
    items i
JOIN 
    items_channels ic ON i.item_id = ic.item_ch_item_id
JOIN 
    channels c ON ic.item_ch_channel_id = c.channel_id;

SELECT uom_shipping_dimension_id, COUNT(*)
FROM uoms
GROUP BY uom_shipping_dimension_id
HAVING COUNT(*) > 1;
